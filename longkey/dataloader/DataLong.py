...
class DataLong(AbstractJoint):
    ...
    def __getitem__(self, index):
        ...
        src_tensor, tot_phrase_len, orig_doc_len = self._converter(
            tokens, valid_mask, phrase_list, self.tokenizer, self.mode
        )

        if self.mode == "train":
            chunk_label = torch.LongTensor(chunk_label)
            ngram_label = torch.LongTensor(ngram_label)
    ...
    def _converter(
        ...
        return src_tensor, tot_phrase_len, orig_doc_len
    ...
    def batchify_train(batch, encoder_output_dim, global_attention):
        ...
        batch_size = len(docs)
        keyphrase_embedding = torch.FloatTensor(batch_size, max_diff_gram_num).fill_(0)
        ...
        valid_output = torch.zeros(len(docs), max_word_len, encoder_output_dim * 1)
